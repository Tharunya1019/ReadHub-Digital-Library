...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 28, 2025 at 04:47 AM
...
(30, 'sf2.jpg', 'The Solar War', 16, 1, '27899', 200, 5, 'Available'),
...
(1, 30, '0000-00-00', '0000-00-00', '', 0);

--
-- Triggers `issueinfo`
--
DELIMITER $$
CREATE TRIGGER `UpdateBookOnReturn` AFTER UPDATE ON `issueinfo` FOR EACH ROW BEGIN
    IF NEW.approve = 'Returned' AND OLD.approve <> 'Returned' THEN
        UPDATE books
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `after_issue_update` AFTER UPDATE ON `issueinfo` FOR EACH ROW BEGIN
  IF NEW.approve <> '' AND OLD.approve = '' THEN
    UPDATE books
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `auto_fine_calculation` BEFORE UPDATE ON `issueinfo` FOR EACH ROW BEGIN
    DECLARE days_late INT;
    DECLARE due_date DATE;
    
    
    IF NEW.approve LIKE '%RETURNED%' THEN
        
        SELECT returndate INTO due_date 
        FROM issueinfo 
        WHERE studentid = NEW.studentid AND bookid = NEW.bookid 
        AND (approve = 'Yes' OR approve LIKE '%EXPIRED%')
        LIMIT 1;
...
(12, 'Tahmid12', 'hello', 'yes', 'admin', '04/23/2021 Friday, 07:24 PM');
